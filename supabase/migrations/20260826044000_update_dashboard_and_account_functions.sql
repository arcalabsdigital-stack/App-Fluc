-- Migration: Update get_accounts_with_balances and get_dashboard_kpi to use natureza_contabil and efeito_caixa
-- Security: SECURITY INVOKER

-- 1. get_dashboard_kpi(p_date_now date)
...
BEGIN
  ...
  FOR acc IN SELECT id, tipo, saldo_inicial, data_saldo_inicial FROM public.accounts WHERE organization_id = v_org_id AND is_active = true LOOP
    ...
    acc_conc := acc_conc + COALESCE((
      SELECT SUM(CASE
        WHEN c.efeito_caixa = 'Entrada'    THEN t.amount_paid
        WHEN c.efeito_caixa = 'Saida'      THEN -t.amount_paid
        WHEN c.efeito_caixa = 'Sem_efeito' THEN 0
        WHEN t.type = 'Receita'            THEN t.amount_paid
        ELSE -t.amount_paid
      END)
      FROM public.transactions t
      LEFT JOIN public.categories c ON c.id::text = t.category
      WHERE t.account_id = acc.id AND t.date <= p_date_now AND t.date >= acc.data_saldo_inicial AND t.is_conciliated = true
    ), 0);
    ...
    acc_real := acc_real + COALESCE((
      ...
      FROM public.transactions t
      LEFT JOIN public.categories c ON c.id::text = t.category
      WHERE t.account_id = acc.id AND t.date <= p_date_now AND t.date >= acc.data_saldo_inicial
    ), 0);

    -- Projected (realized + remaining pending)
    acc_proj := acc_real + COALESCE((
      SELECT SUM(CASE
        WHEN c.efeito_caixa = 'Entrada'    THEN (t.amount - t.amount_paid)
        WHEN c.efeito_caixa = 'Saida'      THEN -(t.amount - t.amount_paid)
        WHEN c.efeito_caixa = 'Sem_efeito' THEN 0
        WHEN t.type = 'Receita'            THEN (t.amount - t.amount_paid)
        ELSE -(t.amount - t.amount_paid)
      END)
      FROM public.transactions t
      LEFT JOIN public.categories c ON c.id::text = t.category
      WHERE t.account_id = acc.id AND t.date <= p_date_now AND t.date >= acc.data_saldo_inicial AND t.status != 'pago'
    ), 0);
  ...
  SELECT COALESCE(SUM(t.amount_paid), 0) INTO v_month_income_realized
  FROM public.transactions t
  LEFT JOIN public.categories c ON c.id::text = t.category
  WHERE (c.natureza_contabil = 'Receita' OR (c.natureza_contabil IS NULL AND t.type = 'Receita'))
    AND t.date >= v_start_month AND t.date <= v_end_month AND t.organization_id = v_org_id;

  SELECT COALESCE(SUM(t.amount - t.amount_paid), 0) INTO v_month_income_projected
  FROM public.transactions t
  LEFT JOIN public.categories c ON c.id::text = t.category
  WHERE (c.natureza_contabil = 'Receita' OR (c.natureza_contabil IS NULL AND t.type = 'Receita'))
    AND t.status != 'pago' AND t.date >= v_start_month AND t.date <= v_end_month AND t.organization_id = v_org_id;

  SELECT COALESCE(SUM(t.amount_paid), 0) INTO v_month_expense_realized
  FROM public.transactions t
  LEFT JOIN public.categories c ON c.id::text = t.category
  WHERE (c.natureza_contabil = 'Despesa' OR (c.natureza_contabil IS NULL AND t.type = 'Despesa'))
    AND t.date >= v_start_month AND t.date <= v_end_month AND t.organization_id = v_org_id;

  SELECT COALESCE(SUM(t.amount - t.amount_paid), 0) INTO v_month_expense_projected
  FROM public.transactions t
  LEFT JOIN public.categories c ON c.id::text = t.category
  WHERE (c.natureza_contabil = 'Despesa' OR (c.natureza_contabil IS NULL AND t.type = 'Despesa'))
    AND t.status != 'pago' AND t.date >= v_start_month AND t.date <= v_end_month AND t.organization_id = v_org_id;

  SELECT COALESCE(SUM(t.amount_paid), 0) INTO v_last_month_income
  FROM public.transactions t
  LEFT JOIN public.categories c ON c.id::text = t.category
  WHERE (c.natureza_contabil = 'Receita' OR (c.natureza_contabil IS NULL AND t.type = 'Receita'))
    AND t.date >= v_start_last_month AND t.date <= v_end_last_month AND t.organization_id = v_org_id;

  SELECT COALESCE(SUM(t.amount_paid), 0) INTO v_last_month_expense
  FROM public.transactions t
  LEFT JOIN public.categories c ON c.id::text = t.category
  WHERE (c.natureza_contabil = 'Despesa' OR (c.natureza_contabil IS NULL AND t.type = 'Despesa'))
    AND t.date >= v_start_last_month AND t.date <= v_end_last_month AND t.organization_id = v_org_id;
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE((
      ...
      FROM public.transactions t
      LEFT JOIN public.categories c ON c.id::text = t.category
      WHERE t.account_id = a.id AND t.date >= a.data_saldo_inicial
    ), 0) +
