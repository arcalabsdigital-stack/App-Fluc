...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Check locally managed coupons first
    ...
    if (dbCoupon) {
      ...
      if (dbCoupon.usage_limit && dbCoupon.times_used >= dbCoupon.usage_limit) {
        return new Response(JSON.stringify({ valido: false, erro: 'Limite de uso atingido' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
      }
      if (dbCoupon.discount_type === 'PERCENTAGE' && periodo === 'anual') {
        return new Response(JSON.stringify({ valido: false, erro: 'Este cupom é válido apenas para a modalidade mensal' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
      }
      ...
      if (dbCoupon.discount_type === 'PERCENTAGE') {
        discountVal = total * (dbCoupon.discount_value / 100)
      } else {
        discountVal = dbCoupon.discount_value
      }

      return new Response(JSON.stringify({
        valido: true,
        desconto_valor: discountVal,
        desconto_percentual: dbCoupon.discount_type === 'PERCENTAGE' ? dbCoupon.discount_value : Math.round((discountVal / total) * 100),
        novo_total: Math.max(0, total - discountVal)
      }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }

    // Fallback to Asaas API for legacy coupons
